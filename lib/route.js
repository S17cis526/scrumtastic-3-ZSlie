...
Router.prototype.resource = function(route, resource) {
  var db = this.db
  if (resource.list) router.get(route, function(req, res) {resource.list(req,res,db)});
  if (resource.create) router.post(route, function(req, res) {resource.create(req,res,db)});
  if (resource.read) router.get(route + ':id', function(req, res) {resource.read(req,res,db)});
  if (resource.update) router.post(route + '/:id', function(req, res) {resource.update(req,res,db)});
  if (resource.destroy) router.get(route + '/:id/destroy', function(req, res) {resource.destroy(req,res,db)});
}
